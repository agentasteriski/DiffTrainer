...
###COMMANDS###
    ...
    def write_config(self):
        ...
        for spk, (raw_dir, spk_name_box, spk_lang_select, spk_id_select) in self.spk_info.items():
            ...
            if not os.path.isfile(trns):
                print(f"Missing transcriptions.csv in {raw_dir}")
                messagebox.showerror("Missing data", f"transcriptions.csv not found in:\n{raw_dir}\n\nRun the Convert step (liteconvert) on this data first to generate it.")
                return
            with open(trns, "r", newline="", encoding = "utf-8") as csv_file:
                ...
                if len(prefixes) >= 3:
                    ...
                    print(f"Warning: only {len(prefixes)} recording(s) found for {spk}, using all as test prefixes")
                else:
        ...
        if not allspeakers:
            messagebox.showwarning("No speaker data", "No valid speaker data was found. Check the selected data folder.")
            return
    ...
    def binarize(self):
        if not configpath:
            self.label.configure(text="Please select your config file first!")
            return
        try:
    ...
    def train_function(self):
            if not configpath or not ckpt_save_dir:
                self.label.configure(text="Please select your config and the data you would like to train first!")
                return
            try:
                if torch.cuda.is_available():
                    print(f"GPU available: {torch.cuda.get_device_name(0)}")
    ...
    def run_onnx_export(self):
            ...
            export_dir = ckpt_save_dir
            if expselect.get() == 1 and self.aco_folder_dir:
                export_dir = self.aco_folder_dir
            elif expselect.get() == 2 and self.var_folder_dir:
                export_dir = self.var_folder_dir

            if not export_dir:
                print("ERROR: No checkpoint folder selected. Click '2. Select checkpoint folder' (or set the acoustic/variance folder) first, then try ONNX export again.")
                os.chdir(main_path)
                return

            print("ONNX export using folder: " + export_dir)
                
            if drop_on.get() == "on":
                drop_command = onnxexport.drop_speakers(export_dir, dropspk)
                print("Dropping speakers", dropspk.get())
                try: subprocess.run(drop_command, check=True, shell=True)
                except: 
                    print("Error dropping speakers")
            onnxexport.prep_onnx_export(export_dir)
            command = onnxexport.writecmd(export_dir, expselect, drop_on)
            try: subprocess.run(command, check=True, shell=True)
            except: 
                print("Error exporting onnx")
            onnxexport.onnx_cleanup(export_dir)
            print("Done!")
            os.chdir(main_path)
    ...
    def run_onnx_export2(self): 
            ...
            export_dir = ckpt_save_dir
            if expselect2.get() == 1 and self.aco_folder_dir:
                export_dir = self.aco_folder_dir
            elif expselect2.get() == 2 and self.var_folder_dir:
                export_dir = self.var_folder_dir
            ...
            if drop_on2.get() == "on":
                drop_command = onnxexport.drop_speakers(export_dir, dropspk2)
                print("Dropping speakers", dropspk2.get())
                try: subprocess.run(drop_command, check=True, shell=True)
                except: 
                    print("Error dropping speakers")
            onnxexport.prep_onnx_export(export_dir)
            command = onnxexport.writecmd(export_dir, expselect2, drop_on2)
            try: subprocess.run(command, check=True, shell=True)
            except: 
                print("Error exporting onnx")
            onnxexport.onnx_cleanup(export_dir)
