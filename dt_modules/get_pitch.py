...
def get_pitch_rmvpe(wav_data, hop_size, audio_sample_rate, ds_path, interp_uv=True):
    ...
    if rmvpe is None:
        ...
        checkpoint = os.path.join(ds_path, "checkpoints", "rmvpe", "model.pt")
        from modules.pe.rmvpe import RMVPE
        rmvpe = RMVPE(str(checkpoint))
...
def get_pitch_harvest(waveform, samplerate, length, *, hop_size, f0_min=65, f0_max=1100, speed=1, interp_uv=True):
