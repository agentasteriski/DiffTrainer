# Extra Credits

## Packaged with DiffTrainer
- [corpus_segmenter](https://github.com/MLo7Ghinsan/ghin_shenanigans/blob/main/scripts/corpus_segmenter.py)
- assorted legacy code from MLo7Ghinsan (as maintainer of predecessor project)
- logic from [nnsvs_db_converter](https://github.com/UtaUtaUtau/nnsvs-db-converter) used in liteconvert
- logic from [variance-temp-solution](https://github.com/openvpi/MakeDiffSinger/tree/main/variance-temp-solution) used in csv2ds/get_pitch
- fonts:
    - English: [Red Hat Display](https://fonts.google.com/specimen/Red+Hat+Display)
    - Japanese: [M PLUS 2](https://fonts.google.com/specimen/M+PLUS+2)
    - Simplified Chinese: [Noto Sans SC](https://fonts.google.com/noto/specimen/Noto+Sans+SC)
    - Traditional Chinese: [Noto Sans TC](https://fonts.google.com/noto/specimen/Noto+Sans+TC)
- icon: colorized 'hard-drive.svg' from [Zondicons](https://www.zondicons.com/) 

## External downloads
Note: most are actually downloaded from forks.
- [DiffSinger](https://github.com/openvpi/DiffSinger)
- [SOME](https://github.com/openvpi/SOME)
- [pc-nsf-hifigan-44.1k-hop512-128bin-2025.02](https://github.com/openvpi/vocoders/releases/tag/pc-nsf-hifigan-44.1k-hop512-128bin-2025.02)
- [RMVPE](https://github.com/yxlllc/RMVPE/releases/tag/230917)
- [hnsep_240512](https://github.com/yxlllc/vocal-remover/releases/tag/hnsep_240512)
